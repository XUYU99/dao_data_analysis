...
def get_address_type(address):
    try:
        ...
        print(f"{address}地址的code:{code}")
        if code == b'':
            print(f"{address}地址类型: 普通账户")
            return "EOA"
        else:
            print(f"{address}地址类型: Contract")
            return "Contract"
    except Exception as e:
        print(f"地址 {address} 检查失败：{e}")
        return "Unknown"

# 对 voter_address 列中的每个地址进行判断，并新增一列 address_type 保存结果
df["address_type"] = df["address"].apply(get_address_type)

# 输出前几行结果
print(df.head())

# 如有需要，也可以将结果保存到新的 CSV 文件中
df.to_csv("ui_report_address_type.csv", index=False)
